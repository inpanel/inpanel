...
class ACME(object):
    ...
    def __init__(self):
        self.key_size = 4096
        self.account_file = cur_path + '/' + 'account.key'
    ...
    def create_account_key(self, account_file=None):
        ...
        #     create_account_key
        #     account_file = self.account_file
        tt = self._cmd(['openssl', 'genrsa', '-out', account_file, self.key_size], err_msg="Generate account private key Error")
        if tt is None:
            return None
        else:
            return True

    def create_domain_key(self, domain=[]):
        # openssl genrsa 4096 > github.com.key
        file = cur_path + '/' + domain[0] + '.key'
        tt = self._cmd(['openssl', 'genrsa', '-out', file, self.key_size], err_msg="Generate domain private key Error")
        if tt is None:
            return None
        else:
            return True
    ...
    def singature(self):
        # cur_path = os.path.dirname(os.path.realpath(__file__))
        signed_crt = get_acme_crt(cur_path + '/account.key', cur_path + '/dougroup.com.csr', '/var/www/dougroup.com/.well-known/acme-challenge/')
        if signed_crt is not None:
            with open('./dougroup.com.signed.crt', 'w') as f:
                f.write(signed_crt)
...
def main(argv=None):
    ...
    signed_crt = get_acme_crt(args.account_key, args.csr, args.acme_dir, log=LOGGER, CA=args.ca, disable_check=args.disable_check, directory_url=args.directory_url, contact=args.contact)
...
'''
python acme_tiny.py --account-key ./account.key --csr ./dougroup.com.csr --acme-dir /var/www/dougroup.com/.well-known/acme-challenge/ > ./dougroup.com.signed.crt
[root@doudou ssl]# python2.7 acme_tiny.py --account-key ./account.key --csr ./dougroup.com.csr --acme-dir /var/www/dougroup.com/.well-known/acme-challenge/ > ./dougroup.com.signed.crt
Parsing account key...
Parsing CSR...
Found domains: dougroup.com
Getting directory...
Directory found!
Registering account...
Registered!
Creating new order...
Order created!
Verifying dougroup.com...
dougroup.com verified!
Signing certificate...
Certificate signed!
'''