...
> **功能改进:**
>
> - 代码重构：升级 Python 至最新版，同时移除对 Python2 的支持，升级 tornado 及其他依赖包
> - 程序发布形式调整：通过构建单个二进制可执行文件进行分发和运行，安装位置可自行调整，推荐默认位置：/usr/local/inpanel/inpanel
> - 主配置文件移入 /usr/local/etc/inpanel/config.ini，默认自动生成，默认账号/密码 admin
> - 其他数据文件放置在 /usr/local/inpanel/data/ 目录
> - 系统兼容，支持在不同 Linux 发行版和 Mac 系统下的基础功能支持：如系统概览、文件管理、用户管理、进程查看等
> - 运行模式支持后台修改：正式环境、演示模式，（调整到演示模式后不支持再次修改）
> - 增加页面：【关于应用】、【捐助项目】，增加了捐助通道（微信、支付宝扫码）
> - 新的命令脚本，安装位置在 /usr/local/bin/inpanel 用于服务配置和服务控制
> 
> **Bug修复:**
>
> - 待补充
