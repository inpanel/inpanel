...
> - 发布形式调整：通过构建单个二进制可执行文件进行分发和运行，安装位置可自行调整，推荐默认位置：/usr/local/inpanel/inpanel
> - 配置文件调整：移入 /usr/local/etc/inpanel/config.ini，默认自动生成，默认账号/密码 admin
...
> - 文件管理：新增图片预览
> - 新增命令脚本：用于服务配置和服务控制，安装位置在 /usr/local/bin/inpanelctl
>
> **Bug 修复:**
>
> - 待补充
