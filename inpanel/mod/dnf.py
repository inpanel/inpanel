...
from ..templates.sources import load_dnf
_dnf_cfg = load_dnf()
dnf_reporpms = _dnf_cfg['dnf_reporpms']
_DNF_BUILTIN_MIRRORS = _dnf_cfg['dnf_builtin_mirrors']
...
# ==================================================================
# 镜像源管理（持久化到 sources_dnf.json）
# ==================================================================

def _get_sources_file():
    conf_dir = str(_base_config_path).rstrip('/')
    return os.path.join(conf_dir, 'sources_dnf.json')
...
def _load_sources_config():
    ...
    if os.path.isfile(filepath):
        try:
            ...
            pass
    sources = [{'name': m['name'], 'url': m['url'], 'builtin': m.get('builtin', True)} for m in _DNF_BUILTIN_MIRRORS]
...
def web_handler(context, action):
    ...
    # --- 镜像源管理 ---
        ...
        d = Path(config_path)
        if not d.exists():
            context.write({'code': -1, 'msg': '未找到 DNF 配置目录！'})
            return
    ...
    # --- 第三方专用源（复用 yum 的第三方源函数）---
        ...
        context.write(yum._install_third_party(source_id, 'rhel'))
